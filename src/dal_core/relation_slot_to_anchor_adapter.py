...
def adapt_slot_to_anchors(slot: RelationSlotVector) -> RelationSlotAnchorBundle:
    ...
    input_ids = tuple(slot.preserved_trace_ids)
    output_ids = tuple(t for anchor in anchors for t in anchor.trace)

    neutral = IdentityNeutralCheck(
        check_id=f"id_neutral:slot_to_anchor:{slot.vector_id}",
        input_identity_ids=input_ids,
        output_identity_ids=output_ids,
        preserved=set(input_ids).issubset(set(output_ids)),
    )
